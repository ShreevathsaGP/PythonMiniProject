...
class AlgoVisualizer:
    def __init__(self):
        ...
        # algorithm mappings
        self.topics = ["Sorting", "Graph Search", "Maze Solving"]
        self.algo_objects = {self.topics[Options.start_topic]: Sorting(), self.topics[1]: GraphSearch(), self.topics[2]: MazeSolve()}
        ...
        options = self.algo_objects[self.topics[Options.start_topic]].get_options()
    ...
    def reset_topics(self):
        ...
        self.options.kill()
        options = self.algo_objects[new_topic].get_options()
        self.options = UIDropDownMenu(options, options[0], self.option_rect, self.ui_manager)
    ...
    def process_events(self):
        ...
        self.algo_objects[self.algo_dropdown.selected_option].process_slider(self.slider.get_current_value())
    ...
    def run(self):
        ...
        self.previous_option = self.base_option
        while self.running:
            delta_time = self.clock.tick(Options.frame_rate) / 1000.0 # convert ms to s

            # processing
            self.process_events()
            self.ui_manager.update(delta_time)

            # render relevant options
            if self.previous_topic != self.algo_dropdown.selected_option:
                self.reset_topics()
                self.previous_topic = self.algo_dropdown.selected_option
            if self.previous_option != self.options.selected_option:
                self.reset_info()
                self.previous_option = self.options.selected_option
            ...
            self.algo_objects[self.algo_dropdown.selected_option].render(self.window_surface)
            self.algo_objects[self.algo_dropdown.selected_option].step()
